...
#author yanxianghui
import os
from main import commons as m
from main.dbutils import usertable
import json
import yaml
import xlrd
...
def updateuserpasswd(username, newpwd):
	userdb = usertable()
	res = userdb.updateuserpasswd(username,newpwd)
	return res

def get_users():
	userdb = usertable()
	res = userdb.user_query()
	users=[]
	for re in res:
		user={}
		user["name"]=re[0]
...
def adduser(username, userpass,usertype):
	userdb = usertable()
	res = userdb.user_add(username, userpass,usertype)
	return res

def deluser(username):
	userdb = usertable()
	res = userdb.user_del(username)
	return res
